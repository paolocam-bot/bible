...
class MTView(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self.title_label = ctk.CTkLabel(
            self.content_card, text="Seleziona una segnalazione MT", font=ctk.CTkFont(size=22, weight="bold"), justify="left", anchor="w"
        )
        self.title_label.pack(fill="x", anchor="w", padx=25, pady=(25, 5))

        self.desc_label = ctk.CTkLabel(
            self.content_card, text="Usa l'indice o la barra di ricerca per visualizzare le soluzioni di MT.",
        ...
        # Scrollframe per i passaggi
        self.steps_frame = ctk.CTkScrollableFrame(
            self.content_card, label_text="📌 RISOLUZIONE PROBLEMA MT",
    ...
    def aggiorna_dettagli(self, titolo, descrizione, soluzioni):
        ...
        for i, sol in enumerate(soluzioni, 1):
            ...
            lbl_num = ctk.CTkLabel(
                step_container, text=f"PASSAGGIO {i}", font=ctk.CTkFont(size=11, weight="bold"),
                text_color="white", fg_color=("#1f538d", "#246ab7"), corner_radius=4, width=80, height=22
            )
